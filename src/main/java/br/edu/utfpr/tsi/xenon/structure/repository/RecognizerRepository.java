...
public interface RecognizerRepository extends
    ...
    @Query(nativeQuery = true, value =
        """
        SELECT
            SUM(CASE WHEN rc.created_at = CURRENT_DATE - INTERVAL '7 DAY' then 1 else 0 end)
                as sevenDay,
            SUM(CASE WHEN rc.created_at = CURRENT_DATE - INTERVAL '6 DAY' then 1 else 0 end)
                as sixDay,
            SUM(CASE WHEN rc.created_at = CURRENT_DATE - INTERVAL '5 DAY' then 1 else 0 end)
                as fiveDay,
            SUM(CASE WHEN rc.created_at = CURRENT_DATE - INTERVAL '4 DAY' then 1 else 0 end)
                as fourDay,
            SUM(CASE WHEN rc.created_at = CURRENT_DATE - INTERVAL '3 DAY' then 1 else 0 end)
                as threeDay,
            SUM(CASE WHEN rc.created_at = CURRENT_DATE - INTERVAL '2 DAY' then 1 else 0 end)
                as twoDay,
            SUM(CASE WHEN rc.created_at = CURRENT_DATE - INTERVAL '1 DAY' then 1 else 0 end)
                as oneDay,
